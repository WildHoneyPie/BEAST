...
class dataset_processing(Dataset):
        ...
        for key in tqdm(full_data, total=len(full_data)):
            ...
            for song_idx in range(len(full_data[key][fold_idx])):
                ...
                if self.sample_size is None:
                    ...
                    eval_sample_size = int(44100/1024 * 31)
    ...
    def __getitem__(self, index):
        ...
        x = np.transpose(self.data[index], (1, 0)) 
        x = librosa.power_to_db(x, ref=np.max) 

        x = np.transpose(np.array(x), (1, 0))    #instr, T, dmodel
...
if __name__ == '__main__':
    ...
    dataset = audioDataset(data_to_load=['hainsworth'],
