...
GPU = int(sys.argv[2])
LEFT = int(sys.argv[3])
CENTER = int(sys.argv[4])
RIGHT = int(sys.argv[5])

PROJECT_NAME = f'BEAST_L{str(LEFT)}_C{str(CENTER)}_R{str(RIGHT)}'
...
# train_set2, val_set2, test_set2 = dataset2.get_fold(fold=FOLD)
...
beat_tracker = madmom.features.beats.DBNBeatTrackingProcessor(min_bpm=55.0, max_bpm=215.0, fps=FPS, transition_lambda=2, observation_lambda=8, threshold=0.05,online=True)
downbeat_tracker = madmom.features.downbeats.DBNDownBeatTrackingProcessor(beats_per_bar=[3, 4], min_bpm=55.0, max_bpm=215.0, fps=FPS, transition_lambda=16, observation_lambda=12,threshold=0.05,online=True)
...
def train(model, train_loader, optimizer, scheduler, loss_func, loss_tempo, clip, epoch, device):
    ...
    nan_count = []
    for idx, (dataset_key, data, beat_gt, downbeat_gt, tempo_gt) in enumerate(train_loader):
...
def evaluate(model, val_loader, loss_func, loss_tempo, epoch, device):
    ...
    downbeat_DBN_meter = AverageMeter()
    with torch.no_grad():
        for idx, (dataset_key, data, beat_gt, downbeat_gt, tempo_gt) in enumerate(val_loader):
...
for epoch in range(N_EPOCH):
    ...
    print(f'Epoch: {epoch + 1:02} | Time: {epoch_mins}mxf {epoch_secs}s', flush=True)
